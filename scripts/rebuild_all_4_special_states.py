#!/usr/bin/env python3
"""
rebuild_all_4_special_states.py — Final Visual Authority Rebuilder for All 4 Special States

Generates 4 genuinely distinct, standalone character performances:
1. PROXIMITY   — Quiet awareness (attentive side-gaze, calm stance, soft rose heart, NO sparkles)
2. HOVER       — Active engagement (playful tilt, drawstring tug, wide direct sparkling eyes, open smile, emerald glow, 2 sparkles)
3. CLICK_PET   — Tactile reaction (compact bashful posture, hands near face/chest, closed blush squint, heart particles)
4. CELEBRATION — Milestone victory (triumphant raised fist pump, open victory smile, emerald burst, confetti sparkles)

All assets satisfy:
- 512x512 RGBA
- 100% transparent corners
- Grounding baseline Y in [493, 496]
- Standalone complete illustration (ZERO overlays/composites at runtime)
"""
...
def create_heart_particle(size=14, color=(255, 105, 180, 240)):
    ...
    cx, cy = size // 2, size // 2
    # Cute petite anime heart polygon
    pts = [
        (cx, cy + 4),
        (cx - 4, cy),
        (cx - 4, cy - 3),
...
# ==============================================================================
# 1. PROXIMITY — "Meli notices you nearby"
# ==============================================================================
def generate_proximity_asset():
    ...
    # Attentive eyes looking upper-left
    ...
    # NO sparkles in Proximity!
    return canvas

# ==============================================================================
# 2. HOVER — "Meli realizes you are directly interacting with her"
# ==============================================================================
...
# ==============================================================================
# 3. CLICK_PET — "Meli physically reacts to your touch / caught off guard by affection"
# ==============================================================================
def generate_click_pet_asset():
    """
    CLICK_PET:
    - Scrunched upward shoulders & compact bashful silhouette
    - Hands pulled in close towards face/cheeks in a shy reflex
    - Squeezed smiling blush eyes (> < / bashful crescent blush curves)
    - Strong rosy blush across cheeks & nose
    - Petite delighted/shy mouth
    - Warm rose heart pulse + 3 floating heart particles
    """
    base = Image.open("assets/meli/character/states/meli_happy.png").convert("RGBA")
    w, h = 512, 512
    canvas = Image.new("RGBA", (w, h), (0, 0, 0, 0))

    # 1. Compact cute posture: body pasted with a cozy -1px dip
    canvas.paste(base, (0, 0), base)

    # 2. Cute tucked head angle (-1.8 deg)
    head_box = (150, 25, 360, 205)
    head_crop = base.crop(head_box)
    ...
    # 4. Cute shy "3" / petite wavy delighted mouth (ω)
    draw.ellipse([250, 166, 268, 178], fill=(255, 218, 212, 255))
    # Double cute lip curves
    ...
    # 5. Heavy warm blushing cheeks and nose bridge
    ...
    # Cute blush diagonal hatchings
    for dx in [0, 4, 8]:
        draw.line([222 + dx, 153, 226 + dx, 159], fill=(255, 70, 120, 230), width=2)
        draw.line([272 + dx, 152, 276 + dx, 158], fill=(255, 70, 120, 230), width=2)

    # 6. Distinct Bashful Hand Pose: Hands held up shyly near cheeks / upper chest!
    # Left shy hand cupped near cheek (X: 218-232, Y: 178-196)
    ...
    # Right shy hand cupped near cheek (X: 280-294, Y: 176-194)
    ...
    # 7. Radiant Warm Rose Signal Heart Flash
    hx, hy = 259, 184
    ...
    # 8. Three Floating Heart Particle Accents (♥)
    # Left cheek heart
...
# ==============================================================================
# 4. CELEBRATION — "Meli celebrates a meaningful success / milestone"
# ==============================================================================
def generate_celebration_asset():
    """
    CELEBRATION:
    - High-energy triumph victory stance!
    - Right arm raised high with clenched victory fist
    - Bright joyful open victory tooth smile (:D)
    - Emerald Signal Heart triumph burst
    - 4 celebratory sparkle stars & confetti diamonds
    """
    # Load canonical celebration base / complete
    ...
    # 1. Enhance triumphant facial expression: Big bright sparkling excited eyes & joyful smile
    draw.ellipse([230, 131, 247, 155], fill=skin_color)
    draw.ellipse([268, 132, 285, 156], fill=skin_color)

    lash_color = (34, 28, 40, 255)
    draw.arc([228, 127, 249, 147], start=200, end=350, fill=lash_color, width=3)
    draw.arc([266, 128, 287, 148], start=190, end=340, fill=lash_color, width=3)

    draw.ellipse([233, 135, 246, 153], fill=(122, 59, 69, 255), outline=(50, 25, 35, 255))
    draw.ellipse([271, 136, 284, 154], fill=(122, 59, 69, 255), outline=(50, 25, 35, 255))

    draw.ellipse([235, 139, 244, 151], fill=(200, 90, 110, 255))
    draw.ellipse([273, 140, 282, 152], fill=(200, 90, 110, 255))

    # Radiant triumph sparkle catchlights
    draw.ellipse([235, 136, 241, 142], fill=(255, 255, 255, 255))
    draw.ellipse([273, 137, 279, 143], fill=(255, 255, 255, 255))
    draw.ellipse([240, 146, 244, 150], fill=(255, 255, 255, 230))
    draw.ellipse([278, 147, 282, 151], fill=(255, 255, 255, 230))

    # Cheerful raised victory brows
    draw.arc([228, 118, 248, 131], start=210, end=330, fill=(160, 90, 105, 255), width=2)
    draw.arc([267, 119, 287, 132], start=210, end=330, fill=(160, 90, 105, 255), width=2)

    # Big open triumphant smile
    draw.ellipse([248, 164, 270, 182], fill=skin_color)
    mouth_pts = [
        (250, 167),
        (268, 169),
        (266, 180),
    ...
    draw.polygon(mouth_pts, fill=(190, 40, 70, 255), outline=(60, 20, 30, 255))
    draw.polygon([(251, 167), (267, 169), (265, 173), (253, 172)], fill=(255, 255, 255, 255))
    draw.ellipse([254, 176, 264, 182], fill=(255, 120, 150, 255))

    # Radiant rosy victory cheeks
    blush_l = create_glow(34, (255, 100, 140), radius=14, max_alpha=170)
    canvas.paste(blush_l, (214, 145), blush_l)
    blush_r = create_glow(34, (255, 100, 140), radius=14, max_alpha=170)
    canvas.paste(blush_r, (270, 146), blush_r)

    # 2. Brilliant Emerald Signal Heart Triumph Burst
    hx, hy = 259, 183
    aura_outer = create_glow(60, (0, 230, 118), radius=26, max_alpha=180)
    canvas.paste(aura_outer, (hx - 30, hy - 30), aura_outer)
    aura_inner = create_glow(36, (105, 240, 174), radius=16, max_alpha=240)
    canvas.paste(aura_inner, (hx - 18, hy - 18), aura_inner)
    ...
    # 3. Celebratory Sparkles & Confetti Stars around raised fist
    s1 = create_sparkle_diamond(size=18, color=(255, 215, 0, 250))  # Gold sparkle near fist
    canvas.paste(s1, (356, 46), s1)

    s2 = create_sparkle_diamond(size=14, color=(0, 230, 118, 240))  # Emerald sparkle
    canvas.paste(s2, (158, 140), s2)

    s3 = create_sparkle_diamond(size=12, color=(255, 105, 180, 240)) # Pink sparkle
    canvas.paste(s3, (348, 198), s3)

    s4 = create_sparkle_diamond(size=10, color=(255, 255, 255, 255)) # Pure white glint
    canvas.paste(s4, (198, 70), s4)

    return canvas

def main():
    print("=== REBUILDING ALL 4 SPECIAL PERFORMANCE ASSETS ===")

    prox = generate_proximity_asset().resize((512, 512), Image.LANCZOS)
    hover = generate_hover_asset().resize((512, 512), Image.LANCZOS)
    click = generate_click_pet_asset().resize((512, 512), Image.LANCZOS)
    celeb = generate_celebration_asset().resize((512, 512), Image.LANCZOS)

    # Save to assets/
    prox.save("assets/meli/character/special/meli_proximity.png", "PNG")
    hover.save("assets/meli/character/special/meli_hover.png", "PNG")
    click.save("assets/meli/character/special/meli_click_pet.png", "PNG")
    celeb.save("assets/meli/character/special/meli_celebration.png", "PNG")

    # Save to public/special/
    Path("public/special").mkdir(parents=True, exist_ok=True)
    prox.save("public/special/meli_proximity.png", "PNG")
    hover.save("public/special/meli_hover.png", "PNG")
    click.save("public/special/meli_click_pet.png", "PNG")
    celeb.save("public/special/meli_celebration.png", "PNG")
    ...
    print("[SUCCESS] All 4 special assets saved to assets/ and public/")

    # Pairwise difference report across all 4 special states
    imgs = {
